...
function generate_app_types(manifest_data, config) {
	...
	for (const route of manifest_data.routes) {
		...
		/** @type {Map<string, { optional: boolean, matcher: string }>} */
		const child_params = new Map(
			route.params.map((p) => [p.name, { optional: p.optional, matcher: p.matcher }])
		);
		...
		for (const child of manifest_data.routes.filter((r) => r.id.startsWith(route.id))) {
			for (const p of child.params) {
				if (!child_params.has(p.name)) {
					child_params.set(p.name, { optional: true, matcher: p.matcher }); // always optional
				}
			}
		}
		...
		const layout_params = Array.from(child_params)
			.map(([name, { optional, matcher }]) => {
				const type = matcher
					? `MatcherParam<typeof import('${path_to_matcher(matcher)}').match>`
					: 'string';
